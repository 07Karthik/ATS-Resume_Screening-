...
def SignupAction(request):
    if request.method == 'POST':
        ...
        status = 'none'
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'pass', database = 'resumeanalysis',charset='utf8')
        with con:
            cur = con.cursor()
            ...
            for row in rows:
                if row[0] == email:
                    ...
                    break
        if status == 'none':
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'pass', database = 'resumeanalysis',charset='utf8')
...
def extract_skills_from_resume(file_path):
    ...
    # For demonstration, let's assume we're extracting skills using a simple pattern match
    skills = list(set(['Python', 'Java', 'JavaScript', 'Data Analysis', 'Machine Learning', 'Access', 'Oracle', 'Data Analysis', 'Technical', 'Scrum', 'Cloud', 'Python', 'Shell', 'SQL', 'Data Visualization', 'ETL']))
...
def UploadResumeAction(request):
    if request.method == 'POST':
        ...
        skills = extract_skills_from_resume(fs.path(filename))
        
        # Convert the list of skills to a string
        skills_str = ', '.join(skills)
        # Calculate score based on job requirements
        score = getScore(job_id, list(skills))
        # Store resume data in the database
        db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'pass', database = 'resumeanalysis',charset='utf8')
        db_cursor = db_connection.cursor()
        student_sql_query = "INSERT INTO upload_resume(job_id, username, resume_name, upload_date, resume_json, resume_score) VALUES (%s, %s, %s, %s, %s, %s)"
        db_cursor.execute(student_sql_query, (str(job_id), uname, fname, str(today), str(skills), str(score)))
